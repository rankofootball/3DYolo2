...
#        self.anchors = [BoundBox(0, 0, config['ANCHORS'][2*i], config['ANCHORS'][2*i+1]) for i in range(int(len(config['ANCHORS'])//2))]
    ...
    def load_image(self, i):
        image = io.imread(self.images[i]['filename'])[:,:,:,0]
    ...
    def __getitem__(self, idx):
        ...
        x_batch = np.zeros((r_bound - l_bound, self.config['IMAGE_Z'], self.config['IMAGE_H'], self.config['IMAGE_W'], 1))     # input images
...
#                        print (instance_count,grid_x,grid_y,grid_z,best_anchor)
            ...
            if self.norm != None: 
                ...
                for obj in all_objs:
                    if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['zmax'] > obj['zmin']:

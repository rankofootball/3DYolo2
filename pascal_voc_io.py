...
class PascalVocWriter:
    ...
    def genXML(self):
        ...
        z = SubElement(size_part, 'z')
        depth = SubElement(size_part, 'depth')
        width.text = str(self.imgSize[0])
        height.text = str(self.imgSize[1])
        z.text = str(self.imgSize[2])
        if len(self.imgSize) == 4:
            depth.text = str(self.imgSize[3])
        else:
            depth.text = '1'
    ...
    def addBndBox(self, xmin, ymin, zmin, xmax, ymax, zmax, name):
        bndbox = {'xmin': xmin, 'ymin': ymin, 'zmin': zmin, 'xmax': xmax, 'ymax': ymax, 'zmax': zmax}
        bndbox['name'] = name
    ...
    def appendObjects(self, top):
        for each_object in self.boxlist:
            ...
            zmin.text = str(each_object['zmin'])
            xmax = SubElement(bndbox, 'xmax')
            xmax.text = str(each_object['xmax'])
            ymax = SubElement(bndbox, 'ymax')
            ymax.text = str(each_object['ymax'])
            zmax = SubElement(bndbox, 'zmax')
            zmax.text = str(each_object['zmax'])
...
class PascalVocReader:
    ...
    def addShape(self, label, bndbox):
        ...
        zmin = int(bndbox.find('zmin').text)
        xmax = int(bndbox.find('xmax').text)
        ymax = int(bndbox.find('ymax').text)
        zmax = int(bndbox.find('zmax').text)
